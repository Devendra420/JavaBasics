...
import polymorphism.SavingAccount;
        ...
        while (true) {
            ...
            try{
                try(Connection connection = dbManager.getConnection()){
                    try(PreparedStatement preparedStatement = connection.prepareStatement("select * from account where account_number = ?")){
                        preparedStatement.setString(1,accountNumber);
                        try(ResultSet resultSet = preparedStatement.executeQuery()){
                            if(resultSet.next()){
                                Double amount = resultSet.getDouble("balance");
                                Double interestRate = resultSet.getDouble("interest_rate");
                                SavingAccount savingAccount = new SavingAccount(accountNumber,amount,interestRate);
